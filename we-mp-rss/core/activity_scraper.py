...
def _do_scrape() -> None:
    ...
    try:
        for item in valid_items:
            ...
            row.activity_time = item.get("activity_time")
            row.location = item.get("location")
            row.speaker = item.get("speaker")
            row.speaker_title = item.get("speaker_title")
            row.speaker_intro = item.get("speaker_intro")
            row.organizer = item.get("organizer")
            row.description = item.get("description")
            row.cover_image = item.get("cover_image")
            row.source_url = item.get("source_url")
            row.source_type = item.get("source_type")
            row.source_channel = item.get("source_channel", "website")
            row.raw_date_text = item.get("raw_date_text")
            row.mp_name = item.get("mp_name")
            row.publish_time = item.get("publish_time")
            row.registration_required = 1 if item.get("registration_required") else 0
            row.registration_link = item.get("registration_link")
