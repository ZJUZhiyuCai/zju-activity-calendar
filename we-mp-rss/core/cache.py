...
def clear_public_account_cache(mp_id: str) -> bool:
    """清理指定公众号对应的旧 RSS/内容缓存残留。"""
    if not mp_id:
        return True

    cache_dirs = [
        os.path.normpath("data/cache/rss"),
        os.path.normpath("data/cache/content"),
    ]
    cache_prefix = f"{mp_id}_"
    ...
    try:
        for cache_dir in cache_dirs:
            ...
            for filename in os.listdir(cache_dir):
                if cache_prefix not in filename:
                    continue
                file_path = os.path.normpath(os.path.join(cache_dir, filename))
