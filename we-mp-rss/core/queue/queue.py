...
def _get_redis():
    ...
    try:
        ...
        if not cfg.get("redis.enabled", False):
            print_info("Redis 已禁用，队列状态将仅在进程内共享")
            return None
...
class TaskQueueManager:
    ...
    def add_task(self, task: Callable[..., Any], *args: Any, max_retries: int = 3, task_name: str = None, **kwargs: Any) -> bool:
        ...
        log_event("info", "queue task enqueued", queue_tag=self.tag, task_name=display_name)
        return True
        ...
        try:
            while self._is_running:
                ...
                try:
                    ...
                    log_event("info", "queue task started", queue_tag=self.tag, task_name=task_name)
                    ...
                    while retry_count <= max_retries and not success:
                        ...
                        try:
                            ...
                            log_event("info", "queue task completed", queue_tag=self.tag, task_name=task_name, duration=f"{duration:.2f}")
                            success = True
                            ...
                            if retry_count <= max_retries:
                                ...
                                log_event("warning", "queue task retry", queue_tag=self.tag, task_name=task_name, retry_count=retry_count, max_retries=max_retries, error=e)
                            else:
                                # 达到最大重试次数，记录失败
                                with self._thread_lock:
                                    if self._current_task:
                                ...
                                log_event("error", "queue task failed", queue_tag=self.tag, task_name=task_name, max_retries=max_retries, error=e)
        ...
        if redis_client:
            try:
                ...
                if status and 'is_running' in status:
                    info['is_running'] = status['is_running'] == 'true'
                if status and 'pending_count' in status:
                    info['pending_tasks'] = int(status['pending_count'])
            except Exception:
                pass
    ...
    def get_history_page(self, page: int = 1, page_size: int = 10) -> dict:
        page = max(page, 1)
        page_size = max(page_size, 1)

        redis_client = _get_redis()
        if redis_client:
            return self._get_history_page_from_redis(page=page, page_size=page_size)

        with self._thread_lock:
            history = [record.to_dict() for record in reversed(self._history)]
    ...
    def get_management_snapshot(self, page: int = 1, page_size: int = 10) -> dict:
        status = self.get_detailed_status()
        history_page = self.get_history_page(page=page, page_size=page_size)
        return {
            **status,
...
def _build_main_queue() -> TaskQueueManager:
    return TaskQueueManager(tag="文章采集")


def _build_content_queue() -> TaskQueueManager:
...
def get_task_queue() -> TaskQueueManager:
    ...
    with _registry_lock:
        if _task_queue_instance is None:
            _task_queue_instance = _build_main_queue()
            print_info(f"TaskQueue singleton created, instance id: {_task_queue_instance._instance_id}")
    return _task_queue_instance


def get_content_task_queue() -> TaskQueueManager:
    global _content_queue_instance
    if _content_queue_instance is not None:
        return _content_queue_instance

    with _registry_lock:
        if _content_queue_instance is None:
            _content_queue_instance = _build_content_queue()
            print_info(f"ContentTaskQueue singleton created, instance id: {_content_queue_instance._instance_id}")
    return _content_queue_instance


def initialize_default_queues(*, start_workers: bool = False) -> tuple[TaskQueueManager, TaskQueueManager]:
    main_queue = get_task_queue()
    content_queue = get_content_task_queue()
    if start_workers:
        start_default_queues()
    return main_queue, content_queue


def start_default_queues() -> None:
    main_queue = get_task_queue()
    content_queue = get_content_task_queue()
    main_queue.run_task_background()
    content_queue.run_task_background()


def stop_default_queues() -> None:
    main_queue = get_task_queue()
    content_queue = get_content_task_queue()
    main_queue.stop()
    content_queue.stop()
...
def get_queue_runtime_status() -> dict:
    ...
    content_queue = get_content_task_queue()
    return {
        'main_queue': main_queue.get_queue_info(),
        'content_queue': content_queue.get_queue_info(),
    }


class LazyQueueHandle:
    def __init__(self, resolver: Callable[[], TaskQueueManager]):
        self._resolver = resolver

    def _get_manager(self) -> TaskQueueManager:
...
TaskQueue = LazyQueueHandle(get_task_queue)
ContentTaskQueue = LazyQueueHandle(get_content_task_queue)
