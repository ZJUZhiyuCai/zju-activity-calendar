...
class Wx:
    ...
    def __init__(self):
        ...
        self.wx_login_path = os.path.join(self.base_dir, "static", "wx_qrcode.png")
        self.wx_login_url = "/static/wx_qrcode.png"
        self.lock_file_path = os.path.join(self.base_dir, "data", "lock.lock")
        self.lock_path=os.path.dirname(self.lock_file_path)
        self.static_path = os.path.dirname(self.wx_login_path)
    ...
    def GetHasCode(self):
        if os.path.exists(self.wx_login_path):
            return True
        return False
                    ...
                    if switch_account_link.count() > 0:
                        ...
                        try:
                            ...
                            if account_count > 0:
                                ...
                                time.sleep(0.5)
                                self.Close()
                                sys_notice(f"账号切换成功\n- 账号名称: {account_name} \n- 账号ID: {account_id} \n - Token: {token} \n- 过期时间: {exp_time}", str(cfg.get("server.code_title","浙大活动账号切换成功")))
                                return True
                            else:
    ...
    def wxLogin(self, CallBack=None, NeedExit=True):
        ...
        try:
            ...
            # 启动浏览器并打开微信公众平台
            print_info("正在启动浏览器...")
            browser_name = str(cfg.get("gather.browser_type", os.getenv("BROWSER_TYPE", "")))
            login_headless = str(os.getenv("WECHAT_LOGIN_HEADLESS", "False")).lower() == "true"
            ...
            qrcode, qr_tag = self._find_qrcode_locator(page)
            code_src = None
            try:
                ...
                pass
            print("正在生成二维码图片...")
            print(f"qr_selector:{qr_tag}")
            print(f"code_src:{code_src}")
           
            # 使用Playwright截图功能（添加异常处理）
            qrcode.screenshot(path=self.wx_login_path)

            print("二维码已保存为 wx_qrcode.png，请扫码登录...")
            self.HasCode=True
            if os.path.getsize(self.wx_login_path)<=364:
            ...
            page.wait_for_url(re.compile(r"https://mp\.weixin\.qq\.com/cgi-bin/home.*"), timeout=5*60 * 1000)
    ...
    def check_lock(self, timeout: int = 300) -> bool:
        ...
        try:
            ...
            parts = content.split('|')
            lock_pid = int(parts[0]) if parts and parts[0] else 0
            lock_time = float(parts[1]) if len(parts) > 1 else 0

            if lock_time and (time.time() - lock_time > timeout):
