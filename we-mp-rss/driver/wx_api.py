...
class WeChatAPI:
    ...
    def __init__(self):
        ...
        self.qr_code_path = os.path.join(self.base_dir, "static", "wx_qrcode.png")
    ...
    def _get_qr_info_api(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            sessionid = f"{int(time.time() * 1000)}{self._random_suffix()}"
            start_login_response = self.session.post(
            ...
            start_login_response.raise_for_status()
            start_login_data = start_login_response.json()
            if start_login_data.get("base_resp", {}).get("ret") != 0:
                logger.error(f"startlogin失败: {start_login_data}")
                return None

            uuid = self.session.cookies.get("uuid") or self._generate_uuid()
            timestamp = int(time.time() * 1000)
            qr_api_url = f"{self.base_url}/cgi-bin/scanloginqrcode?action=getqrcode&random={timestamp}"
            ...
            if response.status_code == 200:
                ...
                if 'image/' in content_type:
                    ...
                    try:
                        ...
                        return {'qr_url': self.wx_login_url, 'uuid': uuid}
    ...
    def GetCode(self,CallBack=None,Notice=None):
        ...
        result = self.get_qr_code(CallBack,Notice)
        return {
            "code": result.get("code", f"{self.wx_login_url}?t={(time.time())}"),
            "is_exists": result.get("is_exists", self.GetHasCode()),
        }
    def GetHasCode(self):
        if os.path.exists(self.qr_code_path):
            return True
